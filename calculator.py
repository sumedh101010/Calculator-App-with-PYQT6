...
class Window(QWidget):
    ...
    def initUI(self):   # method to set up buttons, labels and layouts
        ...
        for i,button in enumerate(self.buttons):
            ...
            layout.addWidget(button,row+1,col)  # row +1 because on 0th row there is displaying label thats why row +1 and not column +1
            
        #Adding operator buttons
        for i,op_button in enumerate(self.operator_buttons):
            layout.addWidget(op_button,i+1,3)  #3: The column position where the button will be placed. In this case, all buttons are placed in the fourth column (index 3).
    ...
    def number_button_clicked(self):
        ...
        digit = self.sender().text()   # sender method is used that on which button it is clicked and text() is used so as to get what exactly the button is 
            
        if self.current_input=="0":
            self.current_input=digit
        else:
            self.current_input += digit     # since they are string so concatenation is used
